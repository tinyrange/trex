...
func readVMCode(br *rarBitReader) ([]byte, error) {
	...
	if n > maxCodeSize || n == 0 {
		return nil, fmt.Errorf("invalid VM code size: %w", ErrInvalidFilter)
	}
	buf := make([]byte, n)
	...
	if x != buf[0] {
		return nil, fmt.Errorf("VM code checksum mismatch: %w", ErrInvalidFilter)
	}
	return buf, nil
}
...
func (d *decoder29) parseVMFilter(buf []byte) (*filterBlock, error) {
	...
	if flags&0x80 > 0 {
		...
		if n == 0 {
			...
			if n > maxUniqueFilters {
				return nil, fmt.Errorf("unique filter limit exceeded: %w", ErrInvalidFilter)
			}
			if int(n) > len(d.filters) {
				return nil, fmt.Errorf("undefined filter index: %w", ErrInvalidFilter)
			}
		}
		d.fnum = int(n)
	}
	...
	if flags&0x08 > 0 {
		...
		if n > vmGlobalSize-vmFixedGlobalSize {
			return nil, fmt.Errorf("VM global data limit exceeded: %w", ErrInvalidFilter)
		}
		g = make([]byte, n)
