// Adapted from github.com/nwaples/rardecode/v2 v2.4.1.
// Copyright (c) 2015 Nicholas Waples. BSD-2-Clause; see LICENSE.
...
type decodeReader struct {
	matchRemaining, matchDistance int
	...
	size     int    // win length
	buffered int    // bytes already copied while assembling a cross-window filter
	r        int    // index in win for reads (beginning)
	w        int    // index in win for writes (end)
}

func (d *decodeReader) init(f byteReader, ver int, size int, reset, arcSolid bool, unPackedSize int64) error {
	d.outbuf = nil
	d.tot = 0
	d.err = nil
	d.solid = arcSolid
...
func (d *decodeReader) copyBytes(length, offset int) {
	...
	d.matchRemaining = length - (wend - d.w)
	d.matchDistance = offset
	i := (d.w - offset) % d.size
	if i < 0 {
...
func (d *decodeReader) queueFilter(f *filterBlock) error {
	...
	// Normalize the encoded distance before changing coordinate systems.
	f.offset %= d.size
	if f.offset < 0 {
		f.offset += d.size
	}
	// make offset relative to read index (from write index)
	f.offset += d.w - d.r + d.buffered
	// make offset relative to previous filter in list
	for _, fb := range d.fl {
		if f.offset < fb.offset {
			// filter block must not start before previous filter
			return fmt.Errorf("filter starts before a queued filter: %w", ErrInvalidFilter)
		}
		f.offset -= fb.offset
	}
	// Filter lengths are bounded by the dictionary window.
	f.length %= d.size
	if f.length < 0 {
		f.length += d.size
	}
...
func (d *decodeReader) fill() error {
	...
	if d.matchRemaining > 0 {
		d.copyBytes(d.matchRemaining, d.matchDistance)
	}
	if d.w < d.size {
		d.err = d.dec.fill(d)
...
func (d *decodeReader) processFilters() ([]byte, error) {
	...
	for {
		...
		if f.offset != 0 {
			...
			if f.offset < flen {
				return nil, fmt.Errorf("filter overlap: next offset=%d next length=%d consumed=%d output=%d read=%d write=%d buffered=%d window=%d", f.offset, f.length, flen, d.tot, d.r, d.w, d.buffered, d.size)
			}
			f.offset -= flen
			return b, nil
		}
		if f.length != len(b) {
			return nil, fmt.Errorf("chained filter length mismatch: %w", ErrInvalidFilter)
		}
	}
}
...
func (d *decodeReader) decode() error {
	...
	f := d.fl[0]
	if f.offset < 0 {
		return fmt.Errorf("filter overlaps preceding output: %w", ErrInvalidFilter)
	}
	if f.offset > 0 {
