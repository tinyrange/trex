...
// This trex variant extends the Go reader with legacy block derandomization.
package bzip2
...
type reader struct {
	...
	randomized                   bool
	randomRemaining, randomIndex int
}
...
func (bz2 *reader) readFromBlock(buf []byte) int {
	...
	for (bz2.repeats > 0 || bz2.preRLEUsed < len(bz2.preRLE)) && n < len(buf) {
		...
		bz2.preRLEUsed++
		if bz2.randomized {
			if bz2.randomRemaining == 0 {
				bz2.randomRemaining = randomNumbers[bz2.randomIndex]
				bz2.randomIndex = (bz2.randomIndex + 1) % len(randomNumbers)
			}
			bz2.randomRemaining--
			if bz2.randomRemaining == 1 {
				b ^= 1
			}
		}
...
func (bz2 *reader) readBlock() (err error) {
	...
	bz2.randomized = br.ReadBits(1) != 0
	bz2.randomRemaining, bz2.randomIndex = 0, 0
