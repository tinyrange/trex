...
| NTFS | Reads resident and non-resident unnamed data, fragmented and sparse runs, directories and large indexes, hard links, MFT extension records, named data streams, variable sector/cluster geometry, and variable MFT-record sizes. Path lookup is case-insensitive. | Builds complete NTFS **1.1** and **3.1** on-disk formats. Both include boot/MFT metadata, allocation bitmaps, directory indexes, hard links, ordered attribute lists for extension records, DOS short names, timestamps, and labels. The 3.1 builder also emits canonical `$Secure`/`$Extend` metadata and preserves supplied security descriptors. Boot code, hidden sectors, `$LogFile`, and an exact `$UpCase` can be supplied; the Windows 8.1 profile emits its Unicode 6.2 table and uses that same table for index collation. | Mounted NTFS is read-only; there is no in-place editor. These are on-disk version claims, not blanket compatibility claims for every Windows release that used NTFS. The reader is structurally compatible rather than version-gated, but automated coverage is for generated 1.1 and 3.1 images. Compression, EFS encryption, transactional-log replay, reparse-point semantics, quotas, object IDs, and mutation of existing volumes are not implemented. The builder uses 512-byte sectors and clusters and does not create compressed, encrypted, sparse, reparse, or named-stream files. |
...
| WIM | Reads single-file, multi-image WIM trees, security and file metadata, hard links, and chunked uncompressed, XPRESS-Huffman, or LZX resources with lazy cached reads. | Read-only; no WIM writer, split-WIM assembly, solid-resource authoring, or integrity-table repair. |
...
| Minidumps | Reads system architecture, module list, thread list, x86/x64 contexts, stacks, and bounded frame information. | Read-only; not every optional minidump stream or unwinding scheme is interpreted. |
| Event logs | Reads classic `LfLe` event logs and EVTX chunks/BinXML template substitutions; builds an empty classic event-log file. | EVTX output is structured template values rather than full rendered XML; no event-log writer beyond the empty classic log. |
...
| `binary`, `database/ese`, `windows`, `windows/nls`, `firmware/acpi` | Binary, database, locale, and platform formats with optional Starlark adapters. |
