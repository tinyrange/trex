...
| Aladdin ADCR resources | Decodes ADCR03 Huffman/LZ payloads with explicit dictionary file views, size checks and bounded memory. | Loader-code dictionaries are selected by the caller, never executed or replaced with zeroes. No checksum; other ADCR versions remain unsupported. |
...
| Classic StuffIt | Validates archive/header/fork boundaries and CRCs, decodes stored, method13 and method14 forks, and retains separate data/resource forks and Macintosh metadata. | Other compression methods, encryption, StuffIt5 and StuffItX remain unsupported. Self-extractor code is never run. |
...
| InstallShield cabinets | Reads combined InstallShield 4 cabinets and the split metadata/data layouts identified as versions 5–16, including split volumes, external files, file groups, components, shell-object shortcut records, compression/obfuscation, and version-6+ MD5 payload checks. Versions 4, 5, and 6 have direct synthetic round-trip coverage. | Read-only. The legacy v4 support is for the `0x01000004` combined-cabinet family. Layouts outside the implemented families return a typed unsupported-version error; accepting a version does not imply every vendor extension is modeled. |
