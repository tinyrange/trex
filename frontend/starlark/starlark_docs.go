...
var nativeStarlarkTypes = map[string][]string{
	...
	"installer":                {"container", "files", "find(path)", "format", "installscript", "offset", "payload", "plan(locations={}, variables={}, components=None, local_files=None, ranges=None)", "size"},
	...
	"windows.pe":               {"codeview", "data", "disasm(rva, size=256)", "exports", "imports", "info", "messages", "patch(rva, data, update_checksum=True)", "pointer_string_tables(suffix='', minimum=2, maximum=260)", "read(rva, size)", "resources", "sections", "typelibs", "version", "with_resources(resources)"},
	"windows.pdb":              {"age", "dbi_age", "guid", "nearest(rva)", "signature", "symbols"},
...
var nativeStarlarkSignatures = map[string]string{
	...
	"archive.nsis":                      "archive.nsis(file, maximum_bytes=512MiB, maximum_scan=16MiB, maximum_metadata=64MiB, maximum_instructions=1000000) -> nsis",
	"archive.nsis_list":                 "archive.nsis_list(file, maximum_scan=16MiB, maximum_metadata=64MiB, maximum_instructions=1000000) -> record",
	...
	"archive.installer":                 "archive.installer(file, maximum_scan=256MiB, cache=True, maximum_bytes=512MiB) -> installer",
	"archive.installer_probe":           "archive.installer_probe(file, maximum_scan=256MiB, cache=True, maximum_bytes=512MiB) -> dict",
	...
	"archive.aws":                       "archive.aws(file, maximum_records=1M) -> record",
	"archive.rms_variable":              "archive.rms_variable(file, attributes, maximum_records=1M) -> record",
	"archive.vmsbackup_blocks":          "archive.vmsbackup_blocks(file, maximum_blocks=1M, maximum_records=1M, maximum_block_size=16MiB) -> record",
	"archive.vmsbackup":                 "archive.vmsbackup(file, maximum_blocks=1M, maximum_records=1M, maximum_block_size=16MiB, maximum_files=1M, maximum_attributes=256) -> record",
	"archive.arsenic":                   "archive.arsenic(file, maximum_decoded_bytes=256MiB, maximum_block_bytes=16MiB) -> file",
	"archive.lha":                       "archive.lha(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.hunk_objects":              "archive.hunk_objects(file, maximum_records=1M) -> record",
	"archive.hunk_load":                 "archive.hunk_load(file, maximum_records=1M) -> record",
	"archive.bsd_dump":                  "archive.bsd_dump(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.compress":                  "archive.compress(file, maximum_bytes=2GiB, zero_padding=False) -> file",
	"archive.pack":                      "archive.pack(file, maximum_bytes=2GiB) -> file",
	"archive.irix_image":                "archive.irix_image(file, idb, image_name, maximum_entries=1M) -> record",
	"archive.irix_idb":                  "archive.irix_idb(file, maximum_entries=1M) -> record",
	"archive.irix_tape":                 "archive.irix_tape(file) -> record",
	"archive.mac_resource":              "archive.mac_resource(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.compactpro":                "archive.compactpro(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.stuffit":                   "archive.stuffit(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.tome":                      "archive.tome(file, maximum_entries=1M, maximum_decoded_bytes=256MiB) -> record",
	"archive.adcr":                      "archive.adcr(file, dictionary=None, maximum_decoded_bytes=256MiB) -> file",
	"archive.bru":                       "archive.bru(file, maximum_entries=1M) -> record",
	"filesystem.sgi":                    "filesystem.sgi(file) -> record",
	"filesystem.apm":                    "filesystem.apm(file, block_size=512, maximum_entries=1M) -> record",
	"filesystem.efs":                    "filesystem.efs(file, maximum_entries=1M) -> record",
	"filesystem.hfs":                    "filesystem.hfs(file, maximum_entries=1M) -> record",
	...
	"archive.gzip":                      "archive.gzip(file, maximum_bytes=2GiB) -> file",
	"archive.bzip2":                     "archive.bzip2(file, maximum_bytes=2GiB) -> file",
	...
	"filesystem.ntfs_record":            "filesystem.ntfs_record(source, number)",
