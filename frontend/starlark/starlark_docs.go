...
var nativeStarlarkTypes = map[string][]string{
	...
	"directory":             {"fat_short_path(name)", "files", "find(path)", "mkdir(name)", "remove(name)", "set_attributes(name, readonly=False, hidden=False, system=False, archive=False)", "write(name, value)"},
	...
	"runtime.stage_cache":   {"compute(name, source, options, function, args=(), kwargs={})", "stats"},
...
var nativeStarlarkSignatures = map[string]string{
	...
	"runtime.stage_cache":               "runtime.stage_cache() -> runtime.stage_cache",
